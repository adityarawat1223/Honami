...
module.exports = {
    ...
    async execute(interaction) {
        ...
        switch (subcommand) {
            case "set":
                const test = afk.findOne({ userid: interaction.user.id })
                if (!test) {
                    ...
                    const exampleEmbed = new EmbedBuilder().setDescription(`You are already Afk Please Send a message in chat to remove it `)
                    await interaction.reply({ embeds: [exampleEmbed] })
                }
                break
        }
